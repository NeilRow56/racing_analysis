...
export async function getJumpSpeedRatingsAsOfRuns(
  db: Db,
  runnerIds: string[],
  options: {
    source?: string;
...
async function loadRatingTargets(
  ...
  return (await Promise.all(chunks(runnerIds, QUERY_CHUNK_SIZE).map((runnerIdChunk) =>
    db.select({
...
function calculateRatingsForTargets(
  ...
  return new Map(targets.map((target) => {
    ...
    return [target.runnerId, calculateJumpSpeedRating({
  ...
  if (peerStandardKeys.length === 0) {
    return {
      ...
      contextRaceIds: new Set(
        [...standardContexts, ...sameDayContexts].map((row) => row.raceId),
      ),
      sameDayRaceIds: new Set(sameDayContexts.map((row) => row.raceId)),
    };
  }
