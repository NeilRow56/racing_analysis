...
function longshots(lines: string[], contexts: Context[]) {
  ...
  table(lines, YEARS.flatMap((year) => {
    ...
    return [{ label: ">20/1", decimal: 21 }, { label: ">33/1", decimal: 34 }, { label: ">50/1", decimal: 51 }].map((threshold) => {
      ...
      const capped = winners.reduce((sum, entry) => sum + settleSelection(entry.row.outcome, { maxFractionalOdds: CAP_DECIMAL - 1 })!.grossReturn, 0);
...
function metrics(rows: Row[]) {
  const entries = settled(rows), winners = entries.filter((entry) => entry.row.outcome.won), uncappedReturn = winners.reduce((sum, entry) => sum + entry.settlement.grossReturn, 0), cappedReturn = winners.reduce((sum, entry) => sum + settleSelection(entry.row.outcome, { maxFractionalOdds: CAP_DECIMAL - 1 })!.grossReturn, 0), expected = entries.reduce((sum, entry) => sum + 1 / entry.settlement.settlementOddsDecimal, 0);
...
function longshotStats(rows: Row[]) { const value = metrics(rows), winners = settled(rows).filter((entry) => entry.row.outcome.won && entry.settlement.settlementOddsDecimal > CAP_DECIMAL), uncapped = winners.reduce((sum, entry) => sum + entry.settlement.grossReturn, 0), capped = winners.reduce((sum, entry) => sum + settleSelection(entry.row.outcome, { maxFractionalOdds: CAP_DECIMAL - 1 })!.grossReturn, 0); return { share: rate(uncapped, value.uncappedReturn), lost: uncapped - capped }; }
