...
function metrics(rows: Row[]) { const entries = settled(rows), winners = entries.filter((x) => x.row.outcome.won), returns = winners.reduce((s, x) => s + x.settlement.grossReturn, 0), capped = winners.reduce((s, x) => s + Math.min(x.settlement.grossReturn, CAP), 0), expected = entries.reduce((s, x) => s + 1 / x.settlement.settlementOddsDecimal, 0), prices = entries.map((x) => x.settlement.settlementOddsDecimal); return { bets: entries.length, winners: winners.length, strike: rate(winners.length, entries.length), roi: rate(returns - entries.length, entries.length), cappedRoi: rate(capped - entries.length, entries.length), ae: rate(winners.length, expected), meanSp: average(prices), medianSp: quantile(prices, .5) }; }
