...
function metrics(rows: Row[]) { const entries = settled(rows), winners = entries.filter((x) => x.row.outcome.won), returns = winners.reduce((sum, x) => sum + x.settlement.grossReturn, 0), capped = winners.reduce((sum, x) => sum + Math.min(x.settlement.grossReturn, CAP), 0), expected = entries.reduce((sum, x) => sum + 1 / x.settlement.settlementOddsDecimal, 0), prices = entries.map((x) => x.settlement.settlementOddsDecimal); return { bets: entries.length, winners: winners.length, strike: rate(winners.length, entries.length), roi: rate(returns - entries.length, entries.length), cappedRoi: rate(capped - entries.length, entries.length), ae: rate(winners.length, expected), meanSp: average(prices), medianSp: quantile(prices, .5) }; }
